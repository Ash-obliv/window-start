...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        ...
        WM_DPICHANGED => {
            ...
            if dragging_window {
                // 跨 DPI 拖动：直接使用 Windows 在 WM_DPICHANGED 中建议的外框。
                // 旧逻辑用 MonitorFromWindow 误判目标显示器，会反复触发 WM_DPICHANGED 导致窗口不断变大。
                ...
                // 以 WM_DPICHANGED 建议矩形所对应的显示器为准，而非当前窗口关联（可能滞后）。
                let new_dpi_monitor =
                    unsafe { MonitorFromRect(&suggested_rect, MONITOR_DEFAULTTONULL) };
